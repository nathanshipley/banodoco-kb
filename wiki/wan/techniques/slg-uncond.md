...
- **10 seconds faster** reported by Payuyi (2:38 vs 2:28)
...
| **9, 10, 11** | Tested by Payuyi with good results |
...
**Multiple blocks (9, 10, 11):** Tested by Payuyi with good results for prompt following
...
Payuyi conducted detailed testing on March 13, 2025:

**Test prompt:** "A teddy bear walks forward and waves to the camera. The camera follows the teddy bear"

**Results:**
- **With SLG (end 0.3 or 1.0, blocks 9 or 10):** Misses the waving prompt, walks through door then flips around
- **Without SLG:** Waves but morphs the arm into a hand shape
- **With start_percent 0.2, end 1.0:** Fixes the hand to retain image shape AND follows the prompt

**Key insight:** Using `start_percent` instead of just `end_percent` can improve prompt adherence while maintaining quality benefits.

> "if i instead use start percent at 0.2 end 1.0 then it fixes the hand to retain the image's shape and still follows the prompt" — Payuyi, March 13, 2025
...
## Start Percent vs End Percent

**End percent parameter:** Controls what portion of the sampling steps use the block skipping
- **0.3 (30%):** Apply skipping for first 30% of steps, then normal CFG for remaining 70%
- **1.0 (100%):** Apply skipping for all steps

**Start percent parameter:** Controls when to BEGIN applying the block skipping
- **0.0:** Start from the beginning
- **0.2:** Start at 20% through the sampling process

**Payuyi's finding:** Using `start_percent=0.2` with `end_percent=1.0` provided better prompt following than using `end_percent=0.3` alone.

> "end percent 0.3 at the SLG node?" — seitanism#0, March 13, 2025

> "yep" — IllumiReptilien, March 13, 2025

Community testing suggests 30% (0.3) provides good results without over-applying the effect, but starting at 20% may improve prompt adherence.
...
3. Set `end_percent = 0.3` (or experiment with `start_percent = 0.2, end_percent = 1.0`)
4. Connect to sampler

**Note on start_percent:**
> "the start_percent won't work, I don't know why I even have it there" — Kijai, March 13, 2025

> "okay made the start percent work, but it's probably very bad idea with this" — Kijai, March 13, 2025

> "maybe with vid2vid" — Kijai, March 13, 2025

Kijai later made start_percent functional but noted it may only be useful for vid2vid workflows.
...
### Prompt Following Trade-offs

Payuyi's testing revealed that SLG can sometimes interfere with prompt following:
- May miss specific actions (e.g., waving)
- Can be mitigated by adjusting start_percent and end_percent
- Requires experimentation to find optimal settings for specific prompts
...
7. **Consider start_percent:** For better prompt following, try start_percent=0.2 with end_percent=1.0
8. **Test multiple blocks:** Blocks 9, 10, 11 together may work well
...
| **Speed** | 50% slower | No slowdown (10s faster in some tests) |
...
- How does start_percent affect results?
...
| Poor prompt following | Try start_percent=0.2 with end_percent=1.0 |
| Missing specific actions | Adjust start_percent and end_percent balance |
